import { CONTINENTS, PANEL_OPTIONS } from "./config.js";
...
function createSelect(id) {
    ...
    for (const opt of PANEL_OPTIONS) {
...
function createPlotPanel({ continentKey, panelIndex }) {
    ...
    const select = createSelect(`${continentKey}_select_${panelIndex}`);
...
async function createContinentSection(continent, tooltipEl) {
    ...
    const plotPanel1 = createPlotPanel({ continentKey: continent.key, panelIndex: 1 });
    const plotPanel2 = createPlotPanel({ continentKey: continent.key, panelIndex: 2 });
